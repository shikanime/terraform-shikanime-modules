...
resource "tfe_variable" "tfc_run_service_account_email" {
  for_each = {
    shikanime-studio = {
      value = lookup(data.tfe_outputs.default["google-project-shikanime-studio"].values, "service_accounts_email", null)
    }
    shikanime-studio-labs = {
      value = lookup(data.tfe_outputs.default["google-project-shikanime-studio-labs"].values, "service_accounts_email", null)
    }
  }
